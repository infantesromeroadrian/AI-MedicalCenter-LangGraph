...
class MedicalAgentGraph:
    ...
    async def build_consensus(self, state: MedicalGraphState) -> MedicalGraphState:
        """Build an intelligent consensus response from multiple specialist responses."""
        ...
        # Use the enhanced consensus agent to build intelligent consensus
        try:
            consensus_response = await self.consensus_agent.build_intelligent_consensus(
                agent_responses=agent_responses,
                emergency_status=emergency_status,
            ...
            logger.info(f"Built intelligent consensus response with primary specialty: {primary_specialty}")
            
        except Exception as e:
            logger.error(f"Error building intelligent consensus, falling back to simple consensus: {e}")
            # Fallback to simple consensus
            consensus_response = self._build_simple_consensus(
                agent_responses, primary_specialty, emergency_status
            )
    ...
    def _build_simple_consensus(
        self, 
        agent_responses: Dict[str, AgentResponse], 
        primary_specialty: str,
        emergency_status: Dict[str, Any]
    ) -> ConsensusResponse:
        """Fallback method for building simple consensus."""
